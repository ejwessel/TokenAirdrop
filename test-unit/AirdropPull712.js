...
describe("AirdropPull712 Unit Test", () => {
  ...
  before(async () => {
    ...
    rewardDistributorFactory = await ethers.getContractFactory(
      "AirdropPull712"
    );
    rewardDistributor = await rewardDistributorFactory
      .connect(deployer)
      .deploy(mockToken.address, deployer.address);
  ...
  describe("Test Constructor", async () => {
    it("Test Invalid Token Address fails", async () => {
      await expect(
        rewardDistributorFactory
          .connect(deployer)
          .deploy(AddressZero, deployer.address)
    ...
    it("Test Invalid Signer Address fails", async () => {
      await expect(
        rewardDistributorFactory
          .connect(deployer)
          .deploy(mockToken.address, AddressZero)
  ...
  describe("Test Setters", async () => {
    it("Test setting signer by not owner fails", async () => {
      await expect(
        rewardDistributor.connect(recipient1).setSigner(recipient1.address)
    ...
    it("Test setting signer by owner passes", async () => {
      await rewardDistributor
        .connect(deployer)
        .setSigner(rotatedSigner.address);
  ...
  describe("Test Claiming", async () => {
    ...
    it("Test claiming when signer changes passes", async () => {
      ...
      await expect(
        rewardDistributor
          .connect(recipient1)
          .claim(recipientData, sig2.v, sig2.r, sig2.s)
      ).to.not.be.reverted;
    });
    ...
    it("Test Claim event is emitted", async () => {
      ...
      await expect(
        ...
        .withArgs(0, recipient1.address, amount);
    });
