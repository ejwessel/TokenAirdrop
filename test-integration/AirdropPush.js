...
describe("AirdropPush Integration Test", () => {
  ...
  describe("Test Distributions", async () => {
    it("Test distribute distribute to users", async () => {
      ...
      const user1Bal = await FWB.connect(randomUser1).balanceOf(
        randomUser1.address
      );
      expect(user1Bal).to.equal(10);
      const user2Bal = await FWB.connect(randomUser2).balanceOf(
        randomUser2.address
      );
      expect(user2Bal).to.equal(10);
      const user3Bal = await FWB.connect(randomUser3).balanceOf(
        randomUser3.address
      );
    ...
    it("Test distribute distribute to users DAI", async () => {
      ...
      expect(user1Bal).to.equal(10);
      const user2Bal = await DAI.connect(randomUser2).balanceOf(
        randomUser2.address
      );
      expect(user2Bal).to.equal(10);
      const user3Bal = await DAI.connect(randomUser3).balanceOf(
        randomUser3.address
      );
      expect(user3Bal).to.equal(10);
    });
    ...
    it("Test distribute distribute to users USDC", async () => {
      ...
      const user1Bal = await USDC.connect(randomUser1).balanceOf(
        randomUser1.address
      );
      expect(user1Bal).to.equal(10);
      ...
      expect(user2Bal).to.equal(10);
      const user3Bal = await USDC.connect(randomUser3).balanceOf(
        randomUser3.address
      );
      expect(user3Bal).to.equal(10);
    });
    ...
    it("Test distribute distribute to users USDT", async () => {
      ...
      const user1Bal = await USDT.connect(randomUser1).balanceOf(
        randomUser1.address
      );
      expect(user1Bal).to.equal(10);
      const user2Bal = await USDT.connect(randomUser2).balanceOf(
        randomUser2.address
      );
      expect(user2Bal).to.equal(10);
