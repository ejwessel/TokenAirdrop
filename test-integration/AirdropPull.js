...
describe("AirdropPull Integration Test", () => {
  ...
  before(async () => {
    ...
    FWB = await ethers.getContractAt(
      ERC20.abi,
    ...
    user1Sig = await generateSignature(
      process.env.SIGNER,
      DAI.address,
      claimAmt,
      randomUser1.address
    );
    user2Sig = await generateSignature(
      process.env.SIGNER,
      USDC.address,
      claimAmt,
      randomUser2.address
    );
    user3Sig = await generateSignature(
      process.env.SIGNER,
      USDT.address,
      claimAmt,
      randomUser3.address
    );
    user4Sig = await generateSignature(
      process.env.SIGNER,
      FWB.address,
      claimAmt,
      randomUser4.address
  ...
  describe("Test Distributions", async () => {
    ...
    it("Test users can claim", async () => {
      await distributor
        .connect(randomUser1)
        .claim(DAI.address, randomUser1.address, claimAmt, user1Sig);
      await distributor
        .connect(randomUser2)
        .claim(USDC.address, randomUser2.address, claimAmt, user2Sig);
      await distributor
        .connect(randomUser3)
        .claim(USDT.address, randomUser3.address, claimAmt, user3Sig);
      await distributor
        .connect(randomUser4)
        .claim(FWB.address, randomUser4.address, claimAmt, user4Sig);

      const user1Bal = await DAI.connect(randomUser1).balanceOf(
        randomUser1.address
      );
      expect(user1Bal).to.equal(claimAmt);
      const user2Bal = await USDC.connect(randomUser2).balanceOf(
        randomUser2.address
      );
      expect(user2Bal).to.equal(claimAmt);
      const user3Bal = await USDT.connect(randomUser3).balanceOf(
        randomUser3.address
      );
      expect(user3Bal).to.equal(claimAmt);
      const user4Bal = await FWB.connect(randomUser4).balanceOf(
        randomUser4.address
      );
    ...
    it("Test owner is able to claim remaining amounts", async () => {
      ...
      const usdcBal = await USDC.connect(deployer).balanceOf(
        distributor.address
      );
      const usdtBal = await USDT.connect(deployer).balanceOf(
        distributor.address
      );
      const fwbBal = await FWB.connect(deployer).balanceOf(distributor.address);

      const deployerDAISig = await generateSignature(
        process.env.SIGNER,
        DAI.address,
        daiBal,
        deployer.address
      );
      const deployerUSDCSig = await generateSignature(
        process.env.SIGNER,
        USDC.address,
        usdcBal,
        deployer.address
      );
      const deployerUSDTSig = await generateSignature(
        process.env.SIGNER,
        USDT.address,
        usdtBal,
        deployer.address
      );
      const deployerFWBSig = await generateSignature(
        process.env.SIGNER,
        FWB.address,
        fwbBal,
        deployer.address
